...
var Booking = mongoose.models.Booking || mongoose.model("Booking", BookingSchema);

// ../../lib/db/src/schema/facilities.ts
import mongoose2 from "mongoose";
var facilitySchema = new mongoose2.Schema(
  {
    name: { type: String, required: true, unique: true },
    // e.g., 'prajna'
    displayName: { type: String, required: true },
    // e.g., 'The Prajna Space'
...
var Facility = mongoose2.models.Facility || mongoose2.model("Facility", facilitySchema);
...
async function connectDB() {
  ...
  try {
    ...
    cachedPromise = mongoose3.connect(uri, opts).then((conn) => {
...
var ListBookingsQueryParams = objectType({
  date: coerce.string().optional().describe("Filter by date (YYYY-MM-DD)"),
  labName: coerce.string().optional().describe("Filter by lab name")
});
var ListBookingsResponseItem = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
  labName: stringType(),
  date: stringType(),
  startTime: stringType(),
  endTime: stringType(),
  studentCount: numberType(),
  status: enumType(["pending", "approved", "rejected", "cancelled"]),
  rejectionReason: stringType().nullish(),
  createdAt: stringType()
});
var ListBookingsResponse = arrayType(ListBookingsResponseItem);
var createBookingBodyDateRegExp = new RegExp("^\\d{4}-\\d{2}-\\d{2}$");
var createBookingBodyStartTimeRegExp = new RegExp(
  "^([01][0-9]|2[0-3]):[0-5][0-9]$"
);
var createBookingBodyEndTimeRegExp = new RegExp(
  "^([01][0-9]|2[0-3]):[0-5][0-9]$"
);
var CreateBookingBody = objectType({
  idToken: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
  labName: stringType(),
  date: stringType().regex(createBookingBodyDateRegExp),
  startTime: stringType().regex(createBookingBodyStartTimeRegExp),
  endTime: stringType().regex(createBookingBodyEndTimeRegExp),
  studentCount: numberType()
});
var GetBookingParams = objectType({
  id: coerce.string()
});
var GetBookingResponse = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
...
var DeleteBookingParams = objectType({
  id: coerce.string()
});
var DeleteBookingResponse = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
...
var AdminListBookingsQueryParams = objectType({
  status: enumType(["pending", "approved", "rejected", "cancelled"]).optional().describe("Filter by status")
});
var AdminListBookingsResponseItem = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
...
var AdminListBookingsResponse = arrayType(
  AdminListBookingsResponseItem
);
var ApproveBookingParams = objectType({
  id: coerce.string()
});
var ApproveBookingResponse = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
...
var RejectBookingParams = objectType({
  id: coerce.string()
});
var RejectBookingBody = objectType({
  reason: stringType().optional()
});
var RejectBookingResponse = objectType({
  id: stringType(),
  bookerName: stringType(),
  bookerType: enumType(["faculty"]),
  purpose: stringType(),
...
var GetScheduleQueryParams = objectType({
  date: coerce.string().describe("Date in YYYY-MM-DD format")
});
var GetScheduleResponse = objectType({
  date: stringType(),
  labs: arrayType(
    objectType({
      labName: stringType(),
...
var GetBookingStatsResponse = objectType({
  totalBookings: numberType(),
  pendingApprovals: numberType(),
  todayBookings: numberType(),
  labBreakdown: arrayType(
    objectType({
      labName: stringType(),
      count: numberType()
    })
...
var ListFacilitiesResponseItem = objectType({
  id: stringType(),
  name: stringType(),
  displayName: stringType(),
  type: enumType(["lab", "classroom"]),
  capacity: numberType(),
...
var CreateFacilityBody = objectType({
  name: stringType(),
  displayName: stringType(),
  type: enumType(["lab", "classroom"]),
  capacity: numberType(),
...
var UpdateFacilityBody = objectType({
  name: stringType(),
  displayName: stringType(),
  type: enumType(["lab", "classroom"]),
  capacity: numberType(),
...
var DeleteFacilityResponse = objectType({
  id: stringType(),
  name: stringType(),
  displayName: stringType(),
  type: enumType(["lab", "classroom"]),
